...
class Model {
	...
	constructor(settingsPath: string, jsonSettings: VU_MODEL_SETTING_JSON) {
		...
		const absoluteModelFilePath = path.isAbsolute(this.#jsonSettings.model_file_path)
			? this.#jsonSettings.model_file_path
			: path.join(path.dirname(settingsPath), this.#jsonSettings.model_file_path);

		if (!fs.existsSync(absoluteModelFilePath))
			logger.error(
				`GAML model for VU ${this.#jsonSettings.name} can't be found at ${absoluteModelFilePath}. Please check the path in the settings.json file.`,
			);
