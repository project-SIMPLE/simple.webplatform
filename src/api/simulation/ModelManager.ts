...
import { IS_PLATFORM_PACKAGED } from "../index.ts";
...
class ModelManager {
	...
	#initModelsList(): void {
		...
		const learningPackagePath: string = isAbsolute(process.env.LEARNING_PACKAGE_PATH!)
			? process.env.LEARNING_PACKAGE_PATH!
			: path.join(process.cwd(), process.env.LEARNING_PACKAGE_PATH!);

		if (fs.existsSync(learningPackagePath)) {
			directoriesWithProjects.push(learningPackagePath);
		} else {
			logger.error(
				`Couldn't find learning package at ${learningPackagePath}` +
					`\nPlease set an existing path in your '.env' for 'LEARNING_PACKAGE_PATH'`,
			);
		...
		if (process.env.EXTRA_LEARNING_PACKAGE_PATH !== "") {
			directoriesWithProjects.push(
				isAbsolute(process.env.EXTRA_LEARNING_PACKAGE_PATH!)
					? process.env.EXTRA_LEARNING_PACKAGE_PATH!
					: path.join(process.cwd(), process.env.EXTRA_LEARNING_PACKAGE_PATH!),
			);
		}

		if (directoriesWithProjects.length === 0) {
			const errorMessage: string =
				`Couldn't find any learning package folder in your configuration...` +
				`\nPlease set an existing path in your '.env' for 'LEARNING_PACKAGE_PATH' or 'EXTRA_LEARNING_PACKAGE_PATH'` +
				`\n\tCurrent '.env' path :  ${process.cwd()}/.env` +
				`\n\tCurrent 'LEARNING_PACKAGE_PATH' path : ${learningPackagePath}`;
			if (IS_PLATFORM_PACKAGED) {
				// Not logger as the platform close before logger prints
				console.error(`${errorMessage}\nExiting now...`);
				// Closing the app as can't really use it
				process.exit(1);
			} else {
				logger.fatal(errorMessage);
				logger.fatal("\nThe application won't display any VU in the web interface !!");
				logger.fatal("\nThe application won't display any VU in the web interface !!");
				logger.fatal("\nThe application won't display any VU in the web interface !!");
			}
		} else {
						...
						if (fs.existsSync(settingsPath)) {
							...
							switch (settings.type) {
								...
								case "json_settings":
									...
									this.monitorNestedModels.push(this.saveNewModel(settingsPath, settings));
			...
			if (this.models.length === 0) {
				const errorMessage: string =
					`Couldn't find any VU package in your configuration...` +
					`\nPlease verify that you have some 'settings.json' file in learning package folder(s)`;
				if (IS_PLATFORM_PACKAGED) {
					// Not logger as the platform close before logger prints
					console.error(`${errorMessage}\nExiting now...`);
					// Closing the app as can't really use it
	...
	parseCatalog(catalog: VU_CATALOG_SETTING_JSON, settingsPath: string) {
		...
		const cleanedEntry: MIN_VU_MODEL_SETTING_JSON[] = [];
		const cleanedCatalog: MIN_VU_CATALOG_SETTING_JSON[] = [];
			...
			switch (entry.type) {
				...
				case "json_settings":
					try {
						...
						cleanedEntry.push(this.saveNewModel(settingsPath, entry));
					} catch (e) {
