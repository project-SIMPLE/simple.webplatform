...
export class ApcUpsHid extends EventEmitter {
	...
	open(): void {
		...
		const devices = HID.devices().filter((d) => d.vendorId === this.vendorId && d.productId === this.productId);
	...
	private writeU16(reportId: number, value: number): void {
		...
		this.device?.sendFeatureReport([reportId, unsigned & 0xff, (unsigned >> 8) & 0xff]);
	}
	...
	startPolling(intervalMs: number = 5000): void {
		...
		const poll = () => {
			try {
				...
				if (status.onBattery) {
					if (status.batteryCharge <= 10 && (this.lastBatteryCharge === null || this.lastBatteryCharge > 10)) {
						this.emit("battery-critical", status.batteryCharge);
					} else if (status.batteryCharge <= 20 && (this.lastBatteryCharge === null || this.lastBatteryCharge > 20)) {
