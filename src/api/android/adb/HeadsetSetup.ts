...
export class HeadsetSetup {
	...
	private async checkSetting(adb: Adb, namespace: string, param: string, expectedValue: string): Promise<boolean> {
		...
		logger.trace(
			`[${adb.serial}] Checking ${namespace} setting '${param}' = ${result}, should be ${expectedValue} (${result === expectedValue})`,
		);
	...
	private async setSetting(adb: Adb, namespace: string, param: string, value: string) {
		...
		logger.trace(
			`[${adb.serial}] Set ${namespace} setting '${param}' = ${value}: ${result === undefined ? "ok" : "failed"}`,
		);
	...
	private async checkRequiredApps(adb: Adb, serial: string) {
		const REQUIRED_APPS: { packageName: string; apkFile: string; permission: string; needsToStart: boolean }[] = [
			{
				packageName: "tdg.oculuswirelessadb",
				apkFile: "tdg.oculuswirelessadb.apk",
				permission: "android.permission.WRITE_SECURE_SETTINGS",
				needsToStart: false,
			},
			{
				packageName: "eu.project_simple.adbautoenable",
				apkFile: "eu.project_simple.adbautoenable.apk",
				permission: "android.permission.WRITE_SECURE_SETTINGS",
				needsToStart: true,
			},
		];
