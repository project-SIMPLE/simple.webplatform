	...
	constructor(adbManager: AdbManager) {
		...
		this.wsServer.ws("/*", {
			...
			message: async (_ws, message) => {
				try {
					const jsonMessage: { type: string; h264: boolean; h265: boolean; av1: boolean } = JSON.parse(
					...
					if (previousCodec !== this.useH265) {
						...
						await this.adbManager.restartStreamingAll().then(async () => {
							logger.debug("All streams restarted");
							// Ensure video stream are well init after long restart
							await this.resentAllConfigPackage(true, 500).then(() =>
								logger.trace("Config resent after stream restart"),
							);
						});
					}
				} catch (e) {
				...
				try {
					...
					switch (code) {
						...
						default:
							...
							else logger.debug(`Closing connection normally`);
					}
				} catch (err) {
		...
		this.wsServer.ws<{ streamId: string }>("/stream/:id", {
			...
			open: (ws) => {
				...
				if (scrcpyClient) {
					this.resentConfigPackage(scrcpyClient).then(() =>
						logger.debug(`[${streamId}] Config resent for new device socket client`),
					);
				}
			},
			...
			drain: (ws) => {
				...
				logger
					.getChild("Drain")
					.info(
						`[${streamId}] Backpressure drained, streaming should be back to normal, buffered: ${ws.getBufferedAmount()}`,
					);
			},
	...
	private async runSession(
		...
		try {
			...
			if (error instanceof AdbScrcpyExitedError) {
				...
				logger.fatal(`[${streamIp}] Unexpected error in stream session for ${adbConnection.serial}: {error}`, {
					error,
				});
			}
