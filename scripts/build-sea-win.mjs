...
// Toolkit files (headset APKs + custom scrcpy server binary) read at runtime by
// HeadsetSetup and ScrcpyServer via resolveToolkitAsset (keys: `toolkit/<name>`).
...
if (fs.existsSync(toolkitDir)) {
	for (const name of fs.readdirSync(toolkitDir)) {
		...
		if (fs.statSync(full).isFile()) {
			toolkitAssets[`toolkit/${name}`] = path.relative(root, full).replace(/\\/g, "/");
		}
	}
}
