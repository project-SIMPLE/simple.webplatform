var json_state;
        ...
        function createWebSocket(player_ws_port) {
            ...
            socket.onmessage = function(event) {
                ...
                if (json.type == "json_state") {
                    ...
                    document.querySelector("#strategy-view").innerHTML = json_simulation.contents.strategy;
        ...
        document.querySelector("#strategy-button").addEventListener('click', ()=>{
            ...
            var new_strategy_coded
            if (new_strategy=="Nearest") {
                new_strategy_coded = "nearest"
            }
            else if (new_strategy=="Global Average") {
                new_strategy_coded = "global_average"
            }
            else if (new_strategy=="Local Average") {
                new_strategy_coded = "local_average"
            }
            else {
                new_strategy_coded = ""
            }
            socket.send(JSON.stringify({
                "type": "expression",
                "expr": "do change_strategy($id,\""+new_strategy_coded+"\");"
            }))
        });